...
serve(async (req) => {
  ...
  try {
    ...
    const { action, reservationId, amount, paymentMethod, phoneNumber, transactionId, captureReason, captureAmount } = await req.json();
    ...
    switch (action) {
      ...
      case "initiate": {
        ...
        const returnUrl = `${Deno.env.get("FRONTEND_URL") || "https://gomonto.com"}/dashboard?deposit_status=success&reservation=${reservationId}`;
    ...
    console.error("Smart deposit error:", error instanceof Error ? error.message : "Unknown error");
