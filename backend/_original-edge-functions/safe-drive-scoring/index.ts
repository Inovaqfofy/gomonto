...
interface DrivingEvent {
  type: 'speed' | 'harsh_braking' | 'harsh_acceleration' | 'sharp_turn' | 'shock' | 'location';
  value?: number;
  lat?: number;
  lng?: number;
  timestamp: string;
...
serve(async (req) => {
  ...
  try {
    ...
    const { action, sessionId, reservationId, events } = await req.json();

    console.log(`Safe Drive Scoring action: ${action}`, { sessionId, reservationId });

    switch (action) {
      case "start_session": {
        const { renterId, ownerId, vehicleId } = await req.json();
    ...
    console.error("Safe drive scoring error:", error);
